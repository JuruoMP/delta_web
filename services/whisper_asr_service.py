...
class WhisperASRService:
    def __init__(self):
        ...
        if self.enable_speaker_feature and WHISPERX_AVAILABLE and PYANNOTE_AVAILABLE and self.hf_api_key:
            try:
                ...
                logger.info(f"SpeakerFeatureExtractor initialized. Database contains {self.speaker_extractor.get_speaker_count()} speakers.")
            except Exception as e:
                logger.error(f"Failed to initialize SpeakerFeatureExtractor: {e}")
                self.enable_speaker_feature = False
        
        # 验证WhisperX配置
        if self.enable_diarization and not WHISPERX_AVAILABLE:
            logger.warning("WHISPERX_ENABLE_DIARIZATION is True but WhisperX is not installed. Diarization will be disabled.")
            self.enable_diarization = False
        
        # if self.enable_diarization and not self.hf_api_key:
        #     logger.warning("WHISPERX_ENABLE_DIARIZATION is True but HF_API_KEY is not set. Diarization will be disabled.")
        #     self.enable_diarization = False
        ...
        logger.info(f"WhisperASRService initialized. Using {self.library_type} library and {self.model_size} size model.")
        if self.enable_diarization:
            logger.info("Speaker diarization enabled using WhisperX.")
    ...
    def switch_model(self, language):
        ...
        # 如果模型尚未加载或加载失败，则加载它
        if self.models.get(language) is None:
            self._load_model(language)
        
        self.current_language = language
        self.current_model = self.models.get(language)
        if self.current_model is None:
            raise Exception(f"Model for language '{language}' could not be loaded.")
    ...
    def _load_model(self, language):
        ...
        try:
            if self.library_type == 'openai-whisper':
                ...
                if model is None:
                    raise Exception(f"whisper.load_model('{model_name}') returned None.")
    ...
    def _load_whisperx_model(self):
        ...
        """
        logger.info(f"开始加载WhisperX模型, enable_diarization={self.enable_diarization}, WHISPERX_AVAILABLE={WHISPERX_AVAILABLE}")
        # 尝试加载WhisperX模型
        try:
            # 只有在启用说话人区分时才加载WhisperX模型
            if self.enable_diarization and WHISPERX_AVAILABLE:
                logger.info(f"正在加载WhisperX模型: model_name={self.whisperx_model_name}, device={self.device}, compute_type={self.compute_type}, download_root={self.whisperx_model_path}")
                # 加载WhisperX模型
                try:
                    ...
                    logger.info(f"WhisperX模型加载成功: {self.whisperx_model}")
                except Exception as e:
                    logger.error(f"加载WhisperX模型时出错: {e}", exc_info=True)
                    self.whisperx_model = None
                    return
                
                # 加载对齐模型
                try:
                    logger.info(f"正在加载对齐模型: language_code={self.current_language}, device={self.device}")
                    # 针对中文，明确指定对齐模型
                    if self.current_language == 'zh':
                        self.align_model, self.align_metadata = whisperx.load_align_model(language_code=self.current_language, device=self.device, model_name='bofenghuang/whisperx-alignment-model-zh')
                    else:
                        self.align_model, self.align_metadata = whisperx.load_align_model(language_code=self.current_language, device=self.device)
                    logger.info(f"对齐模型加载成功: {self.align_model}")
                except Exception as e:
                    logger.error(f"加载对齐模型时出错: {e}", exc_info=True)
                    self.align_model = None
                    self.align_metadata = None
                
                # 加载说话人区分模型
                try:
                    logger.info(f"正在加载说话人区分模型: model_name={self.diarize_model_name}, device={self.device}, use_auth_token={bool(self.hf_token)}")
                    self.diarize_model = whisperx.diarize.DiarizationPipeline(
                        model_name=self.diarize_model_name,
                        device=self.device,
                        use_auth_token=self.hf_token
                    )
                    logger.info(f"说话人区分模型加载成功: {self.diarize_model}")
                except Exception as e:
                    logger.error(f"加载说话人区分模型时出错: {e}", exc_info=True)
                    self.diarize_model = None
            else:
                logger.warning("WhisperX模型未加载，因为说话人区分功能未启用或WhisperX库不可用")
                logger.warning(f"未加载的原因: enable_diarization={self.enable_diarization}, WHISPERX_AVAILABLE={WHISPERX_AVAILABLE}")
                self.whisperx_model = None
                self.diarize_model = None
        except Exception as e:
            logger.error(f"加载WhisperX模型时出错: {e}", exc_info=True)
    ...
    def query_task(self, task_id, x_tt_logid, file_url, format="mp3", enable_diarization=None):
        ...
        try:
            ...
            logger.info(f"查询任务参数: use_diarization={use_diarization}, WHISPERX_AVAILABLE={WHISPERX_AVAILABLE}, enable_diarization参数={enable_diarization}, self.enable_diarization={self.enable_diarization}")
            
            # 检查是否启用说话人区分
            if use_diarization and WHISPERX_AVAILABLE:
                logger.info("启用说话人区分功能")
                # 使用WhisperX进行转录和说话人区分
                return self._query_task_with_diarization(task_id, x_tt_logid, file_url, format, headers)
            else:
                logger.warning("未启用说话人区分功能")
                logger.warning(f"未启用说话人区分的原因: use_diarization={use_diarization}, WHISPERX_AVAILABLE={WHISPERX_AVAILABLE}")
                # 使用传统Whisper进行转录
                return self._query_task_without_diarization(task_id, x_tt_logid, file_url, format, headers)
        except Exception as e:
            logger.error(f"查询任务时出错: {e}", exc_info=True)
            headers["X-Api-Status-Code"] = "50000000"
            headers["X-Api-Message"] = str(e)
            response = ApiResponse(headers=headers, json_data={"error": str(e)})
            return response

    def transcribe_audio(self, file_path, format="mp3", language="zh", enable_diarization=None):
        """
        Transcribes an audio file synchronously.
        """
        with self.model_lock:
            # Determine language and switch model if necessary
            lang_to_switch = "zh" # default to Chinese
            if language.lower().startswith("en"):
                lang_to_switch = "en"
            
            if self.current_language != lang_to_switch:
                self.switch_model(lang_to_switch)

            # Use the logic from query_task
            task_id = str(uuid.uuid4())
            x_tt_logid = str(uuid.uuid4())
            
            # This is a synchronous call, so we directly call the processing logic.
            # The query_task method contains the actual transcription logic.
            response = self.query_task(task_id, x_tt_logid, file_path, format, enable_diarization)
            
            # Extract and return the result
            if response.headers["X-Api-Status-Code"] == "20000000":
                return response.json()["result"]
            else:
                raise Exception(f"Transcription failed: {response.headers['X-Api-Message']}")
    ...
    def _query_task_without_diarization(self, task_id, x_tt_logid, file_url, format, headers):
        ...
        # 根据不同的库使用不同的转录方式
        if self.library_type == 'openai-whisper':
            logger.info(f"_query_task_without_diarization: self.current_model is {self.current_model}")
            # 使用完整路径
        ...
        # 构建响应
        response = ApiResponse(headers=headers, json_data={
    ...
    def _query_task_with_diarization(self, task_id, x_tt_logid, file_url, format, headers):
        ...
        使用说话人区分的查询任务实现
        """
        logger.info(f"开始使用说话人区分功能进行查询, whisperx_model={bool(self.whisperx_model)}, diarize_model={bool(self.diarize_model)}")
        if not self.whisperx_model:
            logger.warning("WhisperX模型未加载，正在尝试加载...")
            self._load_whisperx_model()
            
        if not self.whisperx_model:
            logger.warning("WhisperX模型加载失败，降级到不使用说话人区分的模式")
            # 如果加载失败，降级到不使用说话人区分的模式
            return self._query_task_without_diarization(task_id, x_tt_logid, file_url, format, headers)
        
        # 检查说话人区分模型是否加载成功
        if not self.diarize_model:
            logger.warning("说话人区分模型未加载，降级到不使用说话人区分的模式")
            return self._query_task_without_diarization(task_id, x_tt_logid, file_url, format, headers)
        
        try:
            logger.info(f"开始处理音频文件: {file_url}")
            # 使用WhisperX转录
            # 使用完整路径
            full_file_path = os.path.abspath(file_url)
            logger.info(f"音频文件完整路径: {full_file_path}")
            audio = whisperx.load_audio(full_file_path)
            logger.info(f"音频加载成功")
            transcribe_result = self.whisperx_model.transcribe(audio, language=self.current_language)
            logger.info(f"转录完成，结果段落数: {len(transcribe_result.get('segments', []))}")
            ...
            try:
                if self.align_model:
                    ...
                    logger.info(f"时间戳对齐完成")
                else:
                    aligned_result = transcribe_result
                    logger.warning("对齐模型未加载，跳过时间戳对齐步骤")
            except AttributeError as e:
                # 如果对齐模型没有metadata属性，则跳过对齐步骤
                logger.warning(f"对齐模型缺少metadata属性，跳过时间戳对齐步骤: {e}")
                aligned_result = transcribe_result
                pass
            except Exception as e:
                # 如果对齐过程中出现其他错误，也跳过对齐步骤
                logger.warning(f"时间戳对齐过程中出现错误，跳过对齐步骤: {e}")
                aligned_result = transcribe_result
                pass
            
            # 进行说话人区分
            logger.info(f"开始说话人区分")
            diarize_segments = self.diarize_model(full_file_path)
            logger.info(f"说话人区分完成，段落数: {len(diarize_segments)}")
            result = whisperx.assign_word_speakers(diarize_segments, aligned_result)
            logger.info(f"说话人分配完成")

            # 提取说话人特征并进行跨录音匹配
            global_speaker_mapping = {}
            if self.enable_speaker_feature and self.speaker_extractor:
                try:
                    logger.info(f"开始提取说话人特征")
                    # 提取当前录音的说话人特征
                    ...
                    logger.info(f"说话人特征提取和匹配完成")
                except Exception as e:
                    logger.error(f"说话人特征提取或匹配时出错: {e}")
            ...
            logger.info(f"标准化结果完成，包含说话人文本: {bool(speaker_texts)}")
            # 构建响应
            response = ApiResponse(headers=headers, json_data={
                "result": standardized_result
            })
